...
class Car(models.Model):
    ...
    def get_service_logs(self):
        return self.logs.filter(services__name__isnull=False).order_by(
            F("timestamp").desc()
        )
    ...
    def get_reminders_overdue(self):
        return (
            ...
            .filter(completed=False)
            .filter(
                Q(date__lte=date.today() - timedelta(days=1))
...
class Fuel(models.Model):
    ...
    log = models.OneToOneField(
        "MileageLog", on_delete=models.CASCADE, related_name="fuel"
    )
...
class Service(models.Model):
    ...
    log = models.ForeignKey(
        "MileageLog", on_delete=models.CASCADE, related_name="services"
    )
...
class Reminder(models.Model):
    ...
    service = models.OneToOneField(
        "Service", on_delete=models.CASCADE, related_name="reminder"
    )
        ...
        return {
            ...
            "service": {
                ...
                "parts": [
                    ...
                    for part in self.service.parts.all()
                ],
            },
        }
